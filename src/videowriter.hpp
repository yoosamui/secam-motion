...
class Videowriter : public ofThread, public VideoWriter
{
  ...
  public:
    ...
    void start()
    {
        ...
        ////                int codec = VideoWriter::fourcc('h', 'v', 'c', '1');  // work but
        ////                crash
        // int codec = VideoWriter::fourcc('X', '2', '6', '4');

        // double fps = 25;  // ofGetFrameRate();
        ...
        // common::log("Create video file = " + filename + " FPS: " + to_string(fps));
        ...
        while (isThreadRunning()) {
            while (m_processing) {
                if (!_commnand_set) {
                    ...
                    //"ffmpeg -i "
                    //"rtsp://admin:'master!31416Pi'@192.168.1.99:554/Streaming/channels/101  "
                    //"-vcodec copy -acodec copy '/home/yoo/ffmpeg-test/MyOutput.mp4' -y ";
                ...
                // string filename = get_filepath("motion_" + m_config.parameters.camname, "jpg",
                // 1);
                // string filename = "/home/yoo/ffmpeg-test/image-" + to_string(idx) + ".jpg";
                // imwrite(filename, prior);
                // m_queue.pop();
            }
            ofSleepMillis(10);
        }
